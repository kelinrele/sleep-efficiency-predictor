...
is scored once. The dataset is synthetic, so everything below describes patterns in generated
...
SleepNet seeds vary by ±0.0007. Neither the GRU history branch nor the decorrelation penalty
...
the models miss: a linear model, gradient boosting and a neural network with a recurrent
history branch all stop at the same place.
...
## 5. Each row pairs a day with the night that follows it

Within a person, efficiency on day t correlates with that row's steps (r = 0.68) and stress
(−0.19). The previous and next days' values show essentially nothing (|r| ≤ 0.015). So each row
reads as "today's activity, tonight's sleep", with no carry-over from yesterday
(`figures/timing_check.png`). This may simply be how the data was generated.
...
grouped-CV R² by at most 0.0003 (`results/feature_retest.csv`).
...
negligible: about −0.06 points at 300 mg, and −0.13 points at 600 minutes. A straight-line steps
term leaves curvature in the residuals, and weakly correlated features absorb some of it. The
small signs for caffeine, screen time and stress all depend on how steps is modelled, so none of
...
the same coefficients, so this is not caused by the two-stage fitting order. In this data,
stress's association with sleep runs through steps. The app flags stress as counterintuitive
and never turns it into advice.
